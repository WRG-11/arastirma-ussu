...
# R89-21b AU-L2-11: tool observations are LLM-fed strings produced by
# external tools (web search, document index, memory). Without an
# explicit boundary, an attacker who can shape tool output (crafted
# URL -> adversarial search-result body) can inject instructions
# directly into the agent's reasoning context. Wrap observations in
# <TOOL_OUTPUT> tags with explicit framing so the model treats the
# wrapped text as data, not instructions. Sister to R89-19b AU-L2-09
# (<USER_INPUT> wrap for translation prompts).
#
# Limitation (documented + sanitized in graph.py): an attacker can
# still inject literal `</TOOL_OUTPUT>` substrings to "close" the
# wrapper early. graph.py.action_node escapes that substring before
# formatting. A fully tamper-resistant scheme would use a per-process
# random tag suffix; deferred per brief Q2 to a future hardening pass.
